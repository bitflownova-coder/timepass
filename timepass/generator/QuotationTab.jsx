...
const QuotationTab = () => {
  ...
  const [formData, setFormData] = useState({
    quotationNumber: `QUO-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,
  ...
  const downloadPDF = async () => {
    try {
      ...
      await storage.addDoc('generator_quotations', quotationData);
      
      // Then download PDF
      html2pdf().set({
        filename: `Quotation_${formData.quotationNumber}.pdf`,
        html2canvas: { scale: 2 },
        jsPDF: { format: 'a4', orientation: 'portrait' }
      }).from(quotationRef.current).save();

      alert('✓ Quotation saved and downloaded successfully!');
    } catch (error) {
      console.error('Error saving quotation:', error);
      alert('Failed to save quotation. PDF will still download.');
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
      ...
      <div className="space-y-4 lg:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          <h3 className="font-bold text-slate-900 mb-4">Quotation Details</h3>
        ...
        <button
          onClick={downloadPDF}
          className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg transition font-medium"
        >
      ...
      <div className="xl:sticky xl:top-24 h-fit">
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="bg-purple-900 text-white px-6 py-3 text-sm font-medium">
            Quotation Preview
          </div>
          
          <div ref={quotationRef} className="p-8 bg-white" style={{ fontFamily: 'Inter, sans-serif' }}>
            {/* Quotation Header */}
            <div className="border-b-4 border-purple-900 pb-6 mb-6">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <h2 className="text-4xl font-light text-purple-300 uppercase tracking-widest">Quotation</h2>
                  <div className="mt-4 space-y-1 text-sm">
                    <div><span className="text-slate-600">No:</span> <span className="font-semibold">{formData.quotationNumber}</span></div>
                    <div><span className="text-slate-600">Date:</span> {formData.date}</div>
                    <div><span className="text-slate-600">Valid Until:</span> {formData.validUntil}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-purple-50 p-4 rounded mb-6">
              <p className="text-xs font-bold text-purple-900 uppercase mb-2">Quote For</p>
            ...
            {formData.termsAndConditions && (
              <div className="bg-slate-50 p-4 rounded mb-4 text-xs">
                <h4 className="font-bold text-slate-900 uppercase mb-2">Terms & Conditions</h4>
                <p className="text-slate-600 whitespace-pre-line">{formData.termsAndConditions}</p>
              </div>
            )}

            {/* Payment Info */}
            <div className="bg-purple-50 p-4 rounded text-xs">
              <h4 className="font-bold text-purple-900 uppercase mb-2">Payment Information</h4>
            ...
            <div className="mt-6 pt-4 border-t border-slate-200 text-xs text-slate-500 text-center">
              Computer Generated Quotation - Bitflow Nova © {new Date().getFullYear()}
            </div>
          </div>
        </div>
      </div>
